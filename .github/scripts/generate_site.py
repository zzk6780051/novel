...
<body>
    ...
    def get_novel_list(self):
        ...
        if not os.path.exists(self.novels_dir):
            print(f"警告：小说目录 '{self.novels_dir}' 不存在")
            return novels
    ...
    def get_chapters(self, novel_path):
        ...
        try:
            ...
            print(f"获取章节列表时出错 {novel_path}: {e}")
        return chapters
    ...
    def read_chapter_content(self, file_path):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # 清理内容，移除可能的乱码和特殊字符
                content = re.sub(r'[^\u4e00-\u9fff\u3000-\u303f\uff00-\uffef\u0000-\u007e\u00a0-\u00ff]', '', content)
                return content
    ...
    def generate_main_index_html(self, novels):
        """生成首页"""
        novels_html = ""
        for novel in novels:
            novel_url = f"{novel['name']}/index.html"
            novels_html += f"""
            <div class="catalog">
                <h2><a href="{novel_url}">{novel['name']}</a></h2>
                <p>共 {len(novel['chapters'])} 章</p>
            </div>
            """
    ...
    def generate_site(self):
        ...
        # 生成首页
        index_html = self.generate_main_index_html(novels)
        with open(os.path.join(self.dist_dir, 'index.html'), 'w', encoding='utf-8') as f:
            f.write(index_html)
        print("已生成首页")
        ...
        for novel in novels:
            ...
            # 生成小说目录页
            novel_index_html = self.generate_novel_index_html(novel)
            with open(os.path.join(novel_dist_dir, 'index.html'), 'w', encoding='utf-8') as f:
                f.write(novel_index_html)
            ...
            for i, chapter in enumerate(novel['chapters']):
                ...
                chapter_path = os.path.join(novel_dist_dir, chapter_filename)
                ...
                try:
                    ...
                    print(f"生成章节文件失败 {chapter_path}: {e}")
            
            print(f"  小说 '{novel['name']}': 生成 {chapter_count} 个章节")
            total_chapters += chapter_count
        
        print(f"站点生成完成！共生成 {len(novels)} 部小说，{total_chapters} 个章节")
